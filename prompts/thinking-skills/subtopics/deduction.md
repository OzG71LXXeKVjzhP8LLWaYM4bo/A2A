...
1. **Content field** contains (in order):
   ...
   - `{{IMAGE}}` placeholder (replaced with actual image after generation)
   - Character statements as HTML `<p><strong>Name:</strong> "quote"</p>`
2. **Image** = Two character portraits side-by-side with names ONLY (NO quotes in image)
...
The content field contains THREE parts in order:
1. Premise wrapped in HTML box
2. `{{IMAGE}}` placeholder (will be replaced with actual image after generation)
3. Character statements as HTML
...
**CRITICAL RULES:**
1. Content = Premise box + `{{IMAGE}}` placeholder + Character statements (in that order)
2. Image = Character portraits with names ONLY (NO quotes/statements in the image)
...
5. ALL deduction questions require images (requires_image: true)
6. Use diverse character pairs - vary genders, ethnicities, names
7. The `{{IMAGE}}` placeholder will be replaced with the actual image after generation
